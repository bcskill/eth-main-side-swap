...
func (ob *Observer) Fetch(startHeight int64) {
	for {
		...
		if err != nil {
			if strings.EqualFold(err.Error(), string("get block info error, err=not found")){
				if ob.Overtaked == false {
					util.Logger.Debugf("%s Has caught up with the latest block",ob.Executor.GetChainName())
					ob.Overtaked = true
				} else {
					util.Logger.Debugf("The interval is too short, the %sObserverFetchInterval needs to be increased",ob.Executor.GetChainName())
				}
			} else {
		...
		if ob.Overtaked {
			if ob.periodDiffHeight() < ob.PeriodHeight {
				ob.PeriodHeight = 0
				ob.Overtaked = false
			} else {
				ob.fetchSleep()
...
func (ob *Observer) fetchBlock(curHeight, nextHeight int64, curBlockHash string) error {
	...
	if err != nil {
		return fmt.Errorf("get block info error, err=%s", err.Error())
	}
